...
def log_in(user_login, user_password):
    ...
    if check_profile in profiles:
        ...
        enter_data(1, 2, 3, 4, true_login)

        root.quit()

        root2 = tkinter.Tk()
        root2.title('Ввод данных')
        root2.geometry('420x600')
        root2.resizable(False, False)
        root2.config(bg='wheat')
...
label_login = tkinter.Label(root, text='Login:', bg='wheat')
label_login.place(x=148.5, y=20)

entry_login = tkinter.Entry(root)
entry_login.place(x=148.5, y=40)

label_password = tkinter.Label(root, text='Password:', bg='wheat')
